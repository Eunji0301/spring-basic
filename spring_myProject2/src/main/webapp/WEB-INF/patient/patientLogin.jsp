...
<%if (msg != "") {
	...
	function check() {
		// 이름으로 객체찾기
		let id = document.getElementsByName("patientId");
		let pw = document.getElementsByName("patientPassword");
		if (id[0].value == "") {
			alert("아이디를 입력해주세요.");
			id[0].focus();
			return;
        ...
        <div class="login-form">
            <form name="frm" onsubmit="check(); return false;">
                <div class="form-group">
                    ...
                    <input type="text" id="username" name="patientId" placeholder="아이디 입력" required>
                </div>
                <div class="form-group">
                    <label for="password">비밀번호</label>
                    <input type="password" id="password" name="patientPassword" placeholder="비밀번호 입력" required>
                </div>
                <div class="form-actions">
                    <button type="submit" class="login-btn" onclick="check();">로그인</button>
                    <div class="links">
                        <a href="#" class="forgot-password">Forgot Password?</a>
                        <a href="${pageContext.request.contextPath}/patient/patientSignin.aws" class="signin-btn">회원가입</a>
                    </div>
                </div>
            </form>
        </div>
